...
test("MCP cards show full URLs and usable actions at desktop and narrow widths", {
  ...
  try {
    ...
    for(const width of [1058,480]){
      await send('Emulation.setDeviceMetricsOverride',{width,height:1000,deviceScaleFactor:1,mobile:false});
      const layout=await evaluate(`(()=>{const row=document.querySelector('#recent-reviews tbody tr'),visible=n=>{const r=n.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.width>0};return {height:row.getBoundingClientRect().height,text:row.textContent,operation:row.cells[0].textContent,repository:row.cells[1].textContent,paragraphs:row.querySelectorAll('p').length,page:document.documentElement.scrollWidth,scroll:document.querySelector('#recent-reviews .review-table-scroll').scrollWidth,statusVisible:visible(row.querySelector('.request-badge')),actionVisible:visible(row.querySelector('button'))};})()`);
      assert.equal(layout.operation,'PublishBackgroundCommandStreaming');assert.equal(layout.repository,'cline/cline · branch feature');assert.equal(layout.paragraphs,0);assert.ok(width>700?layout.height<50:layout.height<110);assert.match(layout.text,/HTTP 499/);assert.doesNotMatch(layout.text,/Response received|HTTP error/);assert.ok(layout.page<=width+1);assert.equal(layout.statusVisible,true);assert.equal(layout.actionVisible,true);
