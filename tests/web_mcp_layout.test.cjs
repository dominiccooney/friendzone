...
test("MCP cards show full URLs and usable actions at desktop and narrow widths", {
  ...
  try {
    ...
    for(const width of [1058,480]){
      await send('Emulation.setDeviceMetricsOverride',{width,height:1000,deviceScaleFactor:1,mobile:false});
      const layout=await evaluate(`(()=>{const row=document.querySelector('#recent-reviews tbody tr'),link=row.querySelector('.review-target a'),visible=n=>{const r=n.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.width>0};return {height:row.getBoundingClientRect().height,text:row.textContent,operation:row.cells[0].textContent,repository:row.cells[1].textContent,paragraphs:row.querySelectorAll('p').length,page:document.documentElement.scrollWidth,scroll:document.querySelector('#recent-reviews .review-table-scroll').scrollWidth,statusVisible:visible(row.querySelector('.request-badge')),actionVisible:visible(row.querySelector('button')),linkVisible:visible(link),link:link?.href,linkTarget:link?.target,linkRel:link?.rel};})()`);
      assert.equal(layout.operation,'InspectPullRequest');assert.equal(layout.repository,'cline/cline #1234');assert.equal(layout.link,'https://github.com/cline/cline/pull/1234');assert.equal(layout.linkTarget,'_blank');assert.match(layout.linkRel,/noopener/);assert.equal(layout.linkVisible,true);assert.equal(layout.paragraphs,0);assert.ok(width>700?layout.height<50:layout.height<110);assert.match(layout.text,/HTTP 499/);assert.doesNotMatch(layout.text,/Response received|HTTP error/);assert.ok(layout.page<=width+1);assert.equal(layout.statusVisible,true);assert.equal(layout.actionVisible,true);
